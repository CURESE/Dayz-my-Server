...
# ── npm ───────────────────────────────────────────────────────────────────────
NPM_TARGET="11.11.0"
NPM_CURRENT=$(npm --version 2>/dev/null || echo "0.0.0")

if [[ "$NPM_CURRENT" == "$NPM_TARGET" ]]; then
  success "npm already at target version: $NPM_TARGET"
else
  info "Updating npm from $NPM_CURRENT to $NPM_TARGET…"
